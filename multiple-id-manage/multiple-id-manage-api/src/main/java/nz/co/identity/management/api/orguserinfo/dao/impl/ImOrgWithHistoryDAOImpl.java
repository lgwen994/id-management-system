...
/**
 * Get organizational information by access DB.<br>
 *
 * @since Staveware Core Ver.5.3
 */
@Service(value = "stvIdmfOrgWithHistoryDAO")
public class ImOrgWithHistoryDAOImpl implements ImOrgMstDAO {
	/**
	 * serial Generator
	 * 
	 * @since Staveware Core Ver.5.3
	 ...
	 * @since Staveware Core Ver.5.3
	 */
	@Resource(name = "stvIdmfOrgNameMstWithHistoryDAO")
	private ImOrgNameMstDAO stvIdmfOrgNameMstWithHistoryDAO;
	...
	public ImOrgMst registerOrg(ImOrgMst orgMst) {
		...
		if (CollectionUtils.isEmpty(stvIdmfOrgMstList)) {
			...
			Date orgEndTime = orgMst.getActiveEndTime();
			Date orgStartTime = orgMst.getActiveStartTime();
			Date startTime = null;
			Date endTime = null;
			for (ImOrgMst stvIdmfOrgMst : stvIdmfOrgMstList) {
				endTime = stvIdmfOrgMst.getActiveEndTime();
				startTime = stvIdmfOrgMst.getActiveStartTime();

				if (orgStartTime.getTime() > startTime.getTime()) {
					if (endTime == null || orgStartTime.getTime() < endTime.getTime()) {
						throw new ImRuntimeException(
								"There is a record with active start time or active end time covered.");
					}
				} else {
					if (orgEndTime == null || orgEndTime.getTime() > startTime.getTime()) {
						throw new ImRuntimeException(
								"There is a record with active start time or active end time covered.");
					}
				}
			...
			orgMst.setVersionNo(stvIdmfOrgMstMapper.selectMaxVersion(orgMst.getOrgCode()) + 1);
	...
	public boolean deleteOrg(ImOrgMst orgMst) {
		...
		Integer inputVersionNo = orgMst.getVersionNo();
		if (!selectResult.getVersionNo().equals(inputVersionNo)) {
			// 組織コードにより、指定したバージョンが組織マスタに存在しない場合
			throw new ImOptimisticLockingFailureException("There is a record with exclusive error.");
		}
		...
		// 更新するレコードと同じコードを持つレコードの中で最大のバージョン ＋1 の値を、バージョンに設定する。
		orgMst.setVersionNo(stvIdmfOrgMstMapper.selectMaxVersion(selectResult.getOrgCode()) + 1);
		orgMst.setDeletedFlg(Short.valueOf("1"));
		...
		// 論理削除失敗
		if (stvIdmfOrgMstMapper.updateByPrimaryKeySelective(map) == 0) {
			throw new ImRuntimeException("Delete org data by logic from DB with failed.");
		}
		return true;
	}
	...
	public boolean deleteForceOrg(ImOrgMst orgMst) {
		...
		if (!CollectionUtils.isEmpty(stvIdmfOrgNameMstList)) {
			for (ImOrgNameMst stvIdmfOrgNameMst : stvIdmfOrgNameMstList) {
				if (!stvIdmfOrgNameMstWithHistoryDAO.deleteForceOrgName(stvIdmfOrgNameMst)) {
					throw new ImRuntimeException("Delete org name data by logic from DB with failed.");
				}
			}
		}
