 ...
 */
@Service(value = "stvIdmfPositionRoleWithHistoryDAO")
public class ImPositionRoleWithHistoryDAOImpl
    ...
    public ImPositionRole registerPositionRole(
        ...
        if (CollectionUtils.isEmpty(stvIdmfPositionRoleList)) {
            ...
            Date positionRoleEndTime = positionRole.getActiveEndTime();
            Date positionRoleStartTime = positionRole.getActiveStartTime();
            Date starTime = null;
            Date endTime = null;
            for (ImPositionRole stvIdmfPositionRole : stvIdmfPositionRoleList) {
                starTime = stvIdmfPositionRole.getActiveStartTime();
                endTime = stvIdmfPositionRole.getActiveEndTime();

                if (positionRoleStartTime.getTime() > starTime.getTime()) {
                    if (endTime == null || positionRoleStartTime
                            .getTime() < endTime.getTime()) {
                        throw new ImRuntimeException(
                                "There is a record with active start time or active end time covered.");
                    }
                } else {
                    if (positionRoleEndTime == null || positionRoleEndTime
                            .getTime() > starTime.getTime()) {
                        throw new ImRuntimeException(
                                "There is a record with active start time or active end time covered.");
                    }
                }
    ...
    public ImPositionRole updatePositionRole(
        ...
        if (selectResult == null) {
            ...
            if (StringUtils.isEmpty(positionRole.getUpdatedUser())) {
    ...
    public boolean deletePositionRole(ImPositionRole positionRole) {
        ImPositionRole stvIdmfPositionRole = stvIdmfPositionRoleMapper
                .selectByPrimaryKey(positionRole.getPositionRoleId());
        // ポジションIDがポジションに存在しない場合
        if (stvIdmfPositionRole == null) {
            throw new ImRecordInexistenceException(
        ...
        // ポリシIDにより、バージョン 排他処理
        Integer inputVersionNo = positionRole.getVersionNo();
        if (!stvIdmfPositionRole.getVersionNo().equals(inputVersionNo)) {
        ...
        // 更新するレコードと同じコードを持つレコードの中で最大のバージョン ＋1 の値を、バージョンに設定する。
        int version = stvIdmfPositionRoleMapper
                .selectMaxVersion(stvIdmfPositionRole.getPositionRoleCode());
        positionRole.setVersionNo(version + 1);
        positionRole.setDeletedFlg(Short.valueOf("1"));

        Map<String, Object> map = new HashMap<String, Object>();
        map.put("positionRole", positionRole);
        map.put("versionBase", inputVersionNo);
        if (stvIdmfPositionRoleMapper.updateByPrimaryKeySelective(map) == 0) {
            throw new ImRuntimeException(
    ...
    public boolean deleteForcePositionRole(ImPositionRole positionRole) {
        positionRole.setUpdatedTime(CommonUtils.getSystemTime());
        positionRole.setUpdatedUser(CommonUtils.getLoginUser());
        positionRole.setVersionNo(positionRole.getVersionNo() + 1);
        positionRole.setDeletedFlg(Short.valueOf("1"));
