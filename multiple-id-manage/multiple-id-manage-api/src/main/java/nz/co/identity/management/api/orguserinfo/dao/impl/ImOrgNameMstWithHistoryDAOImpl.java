 ...
 */
@Service(value = "stvIdmfOrgNameMstWithHistoryDAO")
public class ImOrgNameMstWithHistoryDAOImpl
    ...
    public ImOrgNameMst registerOrgName(ImOrgNameMst orgNameMst) {
        ...
        if (stvIdmfOrgNameMst != null) {
            throw new ImRuntimeException(
                    "Insert org data into DB with failed because of same locale.");
        } else {
            ...
            if (StringUtils.isEmpty(orgNameMst.getCreatedUser())) {
                orgNameMst.setCreatedUser(CommonUtils.getLoginUser());
            }
            ...
            if (stvIdmfOrgNameMstMapper.insert(orgNameMst) == 0) {
                throw new ImRuntimeException(
                        "Insert orgName data into DB with failed.");
            }
        }
    ...
    public ImOrgNameMst updateOrgName(ImOrgNameMst orgNameMst) {
        ...
        if (selectResult == null) {
            ...
            if (stvIdmfOrgNameMstMapper.updateByPrimaryKey(map) == 0) {
                throw new ImRuntimeException(
                        "Update orgName data into DB with failed.");
            }
        }
    ...
    public boolean deleteOrgName(ImOrgNameMst orgNameMst) {
        ...
        if (StringUtils.isEmpty(locale)) {
            ...
            for (ImOrgNameMst stvIdmfOrgNameMst : stvIdmfOrgNameMstList) {
                versionNo = stvIdmfOrgNameMst.getVersionNo();
                stvIdmfOrgNameMst.setUpdatedTime(CommonUtils.getSystemTime());
                stvIdmfOrgNameMst.setUpdatedUser(CommonUtils.getLoginUser());
                stvIdmfOrgNameMst.setVersionNo(versionNo + 1);
                stvIdmfOrgNameMst.setDeletedFlg(Short.valueOf("1"));

                Map<String, Object> mapOrgName = new HashMap<String, Object>();
                mapOrgName.put("orgNameMst", stvIdmfOrgNameMst);
                mapOrgName.put("versionBase", versionNo);
                if (stvIdmfOrgNameMstMapper
                        .updateByPrimaryKeySelective(mapOrgName) == 0) {
                    throw new ImRuntimeException(
                            "Delete orgName data by logic from DB with failed.");
                }
            }
        } else {
            // ロケールが指定されている場合は、当該ロケールの名称のみ削除する。
            ImOrgNameMst orgName = stvIdmfOrgNameMstMapper
                    .selectByOrgIdByLocale(orgId, locale);
            if (orgName == null) {
                throw new ImRecordInexistenceException(
                        "The orgId[" + orgId + "] and locale[" + locale
                                + "] is not in OrgName.");
            }
            versionNo = orgName.getVersionNo();

            // バージョン 排他処理
            this.checkVersionNo(versionNo, orgNameMst.getVersionNo());

            orgName.setUpdatedTime(CommonUtils.getSystemTime());
            if (StringUtils.isEmpty(orgName.getUpdatedUser())) {
                orgName.setUpdatedUser(CommonUtils.getLoginUser());
            }
            orgName.setVersionNo(versionNo + 1);
            orgName.setDeletedFlg(Short.valueOf("1"));
            orgName.setLocale("");

            Map<String, Object> map = new HashMap<String, Object>();
            map.put("orgNameMst", orgName);
            map.put("versionBase", versionNo);
            if (stvIdmfOrgNameMstMapper.updateByPrimaryKeySelective(map) == 0) {
                throw new ImRuntimeException(
                        "Delete orgName data by logic from DB with failed.");
            }
        }
    ...
    public boolean deleteForceOrgName(ImOrgNameMst orgNameMst) {
        orgNameMst.setUpdatedTime(CommonUtils.getSystemTime());
        orgNameMst.setUpdatedUser(CommonUtils.getLoginUser());
        orgNameMst.setVersionNo(orgNameMst.getVersionNo() + 1);
        orgNameMst.setDeletedFlg(Short.valueOf("1"));
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("orgNameMst", orgNameMst);
